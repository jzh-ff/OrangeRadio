...
pub enum VoiceCommand {
    /// 播放某首歌
    PlaySong { title: String, artist: Option<String> },
    /// 播放歌单
    PlayPlaylist { name: String },
    /// 切换播放模式
    SetMode { mode: orange_core::player::PlaybackMode },
    /// 调音量
...
impl VoiceAssistant {
    ...
    pub async fn transcribe(&self, _audio: &[f32]) -> orange_core::Result<String> {
        Err(orange_core::CoreError::AiService("ASR 尚未实现 (v0.5)".into()))
    }

    /// 将文本解析为结构化指令
    pub async fn parse_command(&self, _text: &str) -> orange_core::Result<VoiceCommand> {
        Err(orange_core::CoreError::AiService("语音指令解析尚未实现 (v0.5)".into()))
    }
}
